...
> Rust编译器非常智能，能让编译器做的，尽量不要手动去做，能在编译时确定的事情，尽量不要放在运行时确定。Rust的设计要点，就是尽量和编译器好好沟通，让其尽可能多地帮着做一些至关重要的事情。我们可以把Rust编译器想象成一个AI，程序员要做的就是和这个AI好好说话，用他能听懂的语言告诉他，我们想要做什么，他就会帮我们做什么。Rust编译器未来一定会融入人工智能技术，Rust语言或基于Rust的变种语言一定会越来越智能。将Rust的思路发扬光大，走得更远一点，尽可能挖掘、榨干编译器的能力，可让编译器进化为“软件工厂”。举几个例子，Rust编译器正在帮我们做的事情
    
> 1、生命周期，所有者权限，结合在一起，可在编译时，分析出每一个变量从哪里来，要到哪里去，由此实现编译时的内存管理，抛弃运行时的垃圾回收器机制。
    
> 2、对代码逻辑的分析，比如分析if...else...的各种路径组合，找出明显的逻辑错误。
    
> 3、对代码正确性的形式化验证、程序性能的优化。（这一块的前景是无限的，是可以和人工智能技术紧密结合的，如果仅仅把“没有GC”当成是Rust的核心设计理念，那就错了，我们需要看到更广阔的前景）
